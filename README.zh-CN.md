...
哪怕跑在同一台机器、共用同一个 Claude 账号，每个 bot 都是相互隔离的沙盒：

- **独立 plugin cache**：每个 bot 有自己的 plugin 副本和版本号。你可以对
  不同 bot 应用不同的 plugin patch，或者并排跑不同版本。
- **独立 memory**：每个 bot 在自己的 `.agent-config/projects/` 下维护长期
  记忆。Orchestrator 看不到 Synthesizer 的笔记，Critic 不会继承 Scout 的
  研究草稿。**人设不串味**。
- **独立 Discord 身份**：每个 bot 是一个不同的 Discord Application，自己的
  token、自己的 allowlist、自己的 approved DM 频道。一个 bot 跟陌生人 pair
  上了，对其他 bot 完全没影响。
- **独立 settings**：permission allowlist、enabled plugins、theme、hooks
  都是 per-bot。你可以在同一个项目里把一个 bot 权限锁严，另一个放宽。
- **跨 provider 共存**：Claude Code bot 和 Codex CLI bot 可以作为兄弟目录
  共存在 `agents/` 下——它们不共用任何配置文件，因为各自有自己的
  `CLAUDE_CONFIG_DIR`（或 `CODEX_HOME` 等）。
- **故障不会传染**：一个 bot 的 plugin server 崩了、Claude session 退出了、
  token 被 revoke 了——其他 bot 照常运行。

机制上，隔离全靠每个 bot 启动 alias 导出的三个环境变量：
...
这就是全部秘密——让 Claude Code（或任何 agent runtime）认为该目录就是它的
"全宇宙"。`/agent-setup-mac` / `/agent-setup-wsl` skill 在你加 bot 时会自动
把这三个变量配好。
