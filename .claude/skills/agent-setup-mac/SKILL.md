...
### Step 7 — Add the launch alias to ~/.zshrc
...
if ! grep -qF "alias ${ALIAS}=" ~/.zshrc 2>/dev/null; then
  echo "$ALIAS_LINE" >> ~/.zshrc
fi
```

If `.zshrc` doesn't exist or the user is on bash, ask which shell file
they want (`.zshrc`, `.bashrc`, or "just print the alias").
...
3. In a fresh terminal:  source ~/.zshrc  &&  <alias>
